'''
타이타닉 데이터셋 기초 탐색
pandas head/tail/shape/info/colunms 를 사용해서 데이터셋의 기본 정보를 화면에 순서대로
보여주는 streamlit run day03-01.py
'''
import io
import pandas as pd
import streamlit as st
st.title('football player ')
...
if df is not None : 
    ...
    # df.info 는 값을 리턴하지 않고 화면에 직접 출력만 해주는 함수라서 is.StringIO()라는 '메모리 위에 가짜 파일'에
    # 결과를 받아낸 뒤 그 내용을 text로 보여준다
    buffer = io.StringIO()
    df.info(buf=buffer)
    ...
    st.success('기초정보확인 끝났습니다. 다음 예제에서 전처리 필러팅을 할게요')
